package de.akquinet.myHoliday.dao.common;
 ...
 */
public abstract class JpaGenericDao<E extends Serializable> implements
		GenericDao<E> {
	...
	public List<E> loadAll() {
		...
		final TypedQuery<E> query = this.entityManager.createQuery("SELECT e FROM "
				+ persistenceClass.getSimpleName() + " e", persistenceClass);
	...
	protected E loadByQuery(final String stmt) {
		return this.entityManager.createQuery(stmt, getPersistentClass())
				.getSingleResult();
	}
	 ...
	 */
	protected List<E> findByQuery(final String stmt,
			final Map<String, Object> parameters, final int maxResults) {

		checkMaxResult(maxResults);

		final TypedQuery<E> query = this.entityManager.createQuery(stmt,
				getPersistentClass());
	 ...
	 * @return das Object
	 */
	protected E loadByNamedQuery(final String queryName,
			final Map<String, Object> parameters) {
		final TypedQuery<E> namedQuery = this.entityManager.createNamedQuery(
	 ...
	 * @return das Object
	 */
	protected E findSingleByNamedQuery(final String queryName,
			final Map<String, Object> parameters) {
		final TypedQuery<E> namedQuery = this.entityManager.createNamedQuery(
				queryName, getPersistentClass());
		addParameters(namedQuery, parameters);
	 ...
	 */
	protected List<E> findByNamedQuery(final String queryName,
			final Map<String, Object> parameters) {
		final TypedQuery<E> query = this.entityManager.createNamedQuery(queryName,
				getPersistentClass());
		addParameters(query, parameters);
		return query.getResultList();
	}
	...
	/**
	 * Führt das NamedQuery <tt>queryName</tt> mit den Parametern <tt>parameters</tt> aus und liefert ab der Zeile <tt>firstResult</tt> aber maximal <tt>maxResults</tt> Zeilen.
	 * 
	 * @param queryName
	 *          der Name des Querys
	 * @param parameters
	...
	public List<E> findByNamedQuery(final String queryName,
			final Map<String, Object> parameters, final int firstResult,
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected List<Object> findByNativeQuery(final String stmt,
			final Map<String, Object> parameters) {
		return addParameters(this.entityManager.createNativeQuery(stmt), parameters)
				.getResultList();
	}
	 ...
	 */
	protected int executeUpdate(final String queryName,
			final Map<String, Object> parameters) {
		return addParameters(this.entityManager.createNamedQuery(queryName),
	...
	private void checkMaxResult(final int maxResults) {
		if (maxResults < 1) {
			final String msg = MessageFormat.format(
					"maxResults darf nicht kleiner als 1 sein [{0}]",
					Integer.valueOf(maxResults));
			throw new IllegalArgumentException(msg);
		}
	}

	private static Query addParameters(final Query stmt,
			final Map<String, Object> parameters) {
		if (null == parameters) {
			return stmt;
		}
	 ...
	 * 
	 * @param params
	 *          must be an even number of alternating arguments, where the first argument is a string (key) and the second an arbitrary object (value).
	 * 
	...
	public static Map<String, Object> createParameterMap(final Object... params) {
		...
		for (int i = 0; i < params.length; i += 2) {
			assert i + 1 < params.length : "Not enough parameters "
